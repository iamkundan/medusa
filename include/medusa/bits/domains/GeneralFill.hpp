...
void GeneralFill<vec_t>::operator()(DomainDiscretization<vec_t>& domain, const func_t& h,
    ...
    KDTree<vec_t> boundary_search;
    domain.makeDiscreteContainsStructure(boundary_search);
    ...
    while (cur_node < end_node && end_node < max_points) {
        ...
        for (const auto& f : candidates) {
            ...
            if (!domain.contains(node, boundary_search)) continue;  // If node is not in the domain.
