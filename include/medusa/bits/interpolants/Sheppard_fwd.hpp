 ...
 * By supplying a positive regularization parameter one can avoid singularities at the locations of the
 * data points as well as control the "smoothness" of the interpolation (e.g., make the weights of
 * the neighbors less varied). The "smoothness" of interpolation can also be controlled by the power
 * parameter (`power`).
